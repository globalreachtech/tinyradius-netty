...
class AttributeTypeTest {
    ...
    void createFallbackFieldSizes() throws IOException {
        ...
        IntegerAttribute attribute1 = IntegerAttribute.FACTORY.create(dictionary, 4846, 1, (byte) 1, new byte[4]);
        assertEquals(7, attribute1.toByteArray().length);

        assertFalse(dictionary.getVendor(9999).isPresent()); // default 1,1

        IntegerAttribute attribute2 = IntegerAttribute.FACTORY.create(dictionary, 9999, 1, (byte) 1, new byte[4]);
